        ...
        @foreach($blogs as $blog)
            <a href="{{ route('blog-detail', $blog->blog_id) }}" class="group">
                <div class="bg-white rounded-2xl overflow-hidden border border-transparent
                            ...
                            transition-all duration-500 hover:-translate-y-1 flex flex-col h-full">
                    {{-- Image --}}
                    <div class="aspect-video w-full overflow-hidden relative">
                        <img class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110"
                             src="{{ str_starts_with($blog->blog_image, 'http') ? $blog->blog_image : asset('storage/'. $blog->blog_image) }}" alt="{{ $blog->blog_title }}">
                        {{-- Date badge --}}
                    ...
                    {{-- Content --}}
                    <div class="p-5 sm:p-6 space-y-3 flex flex-col flex-grow">
                        <h3 class="text-lg font-bold leading-tight text-foreground group-hover:text-brand-orange transition-colors line-clamp-2">
                            {{ $blog->blog_title }}
                        </h3>
                        <p class="line-clamp-3 text-sm text-muted-foreground leading-relaxed flex-grow">
                            {{ $blog->blog_long_description }}
                        </p>
                        <div class="flex items-center gap-2 text-brand-orange text-sm font-semibold pt-4">
                            Read More
